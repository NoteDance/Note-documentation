...
class DQN: # define a class for the DQN agent
    ...
    def data_func(self,state_pool,action_pool,next_state_pool,reward_pool,done_pool,batch,p):
        s,a,next_s,r,d=self.pr.sample(state_pool,action_pool,next_state_pool,reward_pool,done_pool,self.epsilon,self.alpha,batch,p)
    ...
    def opt(self,gradient): # optimization function, kernel uses it to optimize parameter
        param=self.optimizer(gradient,self.param) # apply the custom momentum optimizer to update the parameters using the gradient
        return param
