...
class DDPG:
    def __init__(self,hidden_dim,sigma,gamma,tau,actor_lr,critic_lr):
        self.genv=[gym.make('Pendulum-v0') for _ in range(5)]
        state_dim=self.genv[0].observation_space.shape[0]
        action_dim=self.genv[0].action_space.shape[0]
        action_bound=self.genv[0].action_space.high[0]
